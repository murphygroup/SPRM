...
class Features:
    ...
    def __init__(self, im: IMGstruct, mask, output_directory, options):
    ...
    def run(self):
        ...
        self.texture = Feature(filename + "_texture", texture.shape, texture_headers, texture)

        outline_vectors, cell_polygons = getparametricoutline(mask, options)
        write_cell_polygs(im, mask, cell_polygons, output_directory, options)
        shape_vectors = getcellshapefeatures(outline_vectors, options)

        self.shape_vectors = Feature()

    def cluster_channels(
        self, im: IMGstruct, mask: MaskStruct, output_directory: Path, options: Dict
    ) -> np.ndarray:
        """
        ...
        pca_channels = PCA(
            n_components=options.get("num_channelPCA_components"), svd_solver="full"
        )
        ...
        reduced_image = reduced_image.reshape(
            reduced_image.shape[0], keep_shape[1], keep_shape[2], keep_shape[3]
        )
        if options.get("debug"):
        ...
        write_2_file(c, filename + "-channelPCA_summary", im, output_directory, inCells, options)

        return reduced_image

    def voxel_cluster(self, im: IMGstruct, options: Dict) -> np.ndarray:
        """
        ...
        np.random.seed(0)
        sampling = float(options.get("pre-cluster_sampling"))
        samples = math.ceil(sampling * channel_data.shape[0])
        # lower bound threshold on pixels
        if samples < options.get("pre-cluster_threshold"):
            samples = options.get("pre-cluster_threshold")
        ...
        start_time = time.monotonic() if options.get("debug") else None

        voxel_by_cluster = KMeans(
            n_clusters=options.get("Number of Voxel Clusters"), random_state=0
        ).fit(channel_data_random)
        ...
        voxelbycluster_labels = voxelbycluster_labels.reshape(
            keep_shape[1], keep_shape[2], keep_shape[3]
        )
    ...
    def glcmProcedure(
        ...
        texture, texture_names = self.__glcm(im, mask, angle, distances, output_directory, options)
    ...
    def __glcm(
        ...
        # For csv writing
        write_2_csv(
            header, ctexture, filename + "_" + "texture", output_directory, cellidx, options
        )
